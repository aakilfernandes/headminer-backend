...
const url_priority_configs = [
  ...
  new PriorityConfig('twitter_influencify_priority', 'twitter_influencified_at'),
  new PriorityConfig('facebook_counts_add_priority', 'facebook_counts_added_at'),
  new PriorityConfig('reddit_counts_add_priority', 'reddit_counts_added_at')
]
...
module.exports = function incrementArticlesPriorities(articles) {
  ...
  articles.forEach((article) => {
      ...
      article_priority_configs.forEach((config) => {
        if (article[config.at_field] === null || (now - article[config.at_field] > config.wait_ms)) {
      ...
      article.url_pojos.forEach((url_pojo) => {
        url_priority_configs.forEach((config) => {
          if (url_pojo[config.at_field] === null || (now - url_pojo[config.at_field] > config.wait_ms)) {
